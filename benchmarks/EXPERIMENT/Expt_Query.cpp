...
uint64_t ExperimentQuery::the_n = 0;
double ExperimentQuery::denom = 0;

void ExperimentQuery::init(uint64_t thd_id, Workload * h_wl, QueryQueue * query_thd) 
{
	_query_thd = query_thd;
	requests = (experiment_request *) mem_allocator.allocate(sizeof(experiment_request) * g_req_per_query, thd_id);
	part_to_access = (uint64_t *) mem_allocator.allocate(sizeof(uint64_t) * g_part_per_txn, thd_id);
...
void ExperimentQuery::calculateDenom()
{
	assert(the_n == 0);
...
// zeta(2.0, theta).
double ExperimentQuery::zeta(uint64_t n, double theta) 
{
	double sum = 0;
...
uint64_t ExperimentQuery::zipf(uint64_t n, double theta) 
{
	assert(this->the_n == n);
...
void ExperimentQuery::gen_requests(uint64_t thd_id, Workload * h_wl) 
{
	#if CC_ALG == HSTORE

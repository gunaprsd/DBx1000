...
void TPCCQuery::initialize(uint64_t thd_id, Workload * h_wl) {
	double x = (double)(rand() % 100) / 100.0;
	part_to_access = (uint64_t *) 
		mem_allocator.allocate(sizeof(uint64_t) * g_part_cnt, thd_id);
	if (x < g_perc_payment)
		gen_payment(thd_id);
	else 
...
void TPCCQuery::gen_payment(uint64_t thd_id) {
	type = TPCC_PAYMENT;
	if (FIRST_PART_LOCAL)
...
void TPCCQuery::gen_new_order(uint64_t thd_id) {
	type = TPCC_NEW_ORDER;
	if (FIRST_PART_LOCAL)
		w_id = thd_id % g_num_wh + 1;
	else

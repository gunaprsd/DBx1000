...
void tpcc_txn_man::initialize(Thread * h_thd, Workload * h_wl, uint64_t thd_id) {
...
Status tpcc_txn_man::run_payment(TPCCQuery * query) {
	Status rc = OK;
	uint64_t key;
	Record * item;
...
Status tpcc_txn_man::run_new_order(TPCCQuery * query) {
	Status rc = OK;
	uint64_t key;
	Record * item;
	INDEX * index;
...
Status 
tpcc_txn_man::run_order_status(TPCCQuery * query) {
/*	row_t * r_cust;
	if (query->by_last_name) {
...
Status 
tpcc_txn_man::run_delivery(TPCCQuery * query) {
/*
...
Status 
tpcc_txn_man::run_stock_level(TPCCQuery * query) {
	return OK;
}

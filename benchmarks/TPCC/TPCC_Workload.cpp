...
Status tpcc_wl::initialize() {
	Workload::initialize();
	string path = "./benchmarks/";
#if TPCC_SMALL
	path += "TPCC/short_schema.txt";
#else
	path += "TPCC/full_schema.txt";
#endif
...
Status tpcc_wl::init_schema(const char * schema_file) {
...
Status tpcc_wl::initialize_table() {
	num_wh = g_num_wh;
...
Status tpcc_wl::get_txn_manager(TransactionManager *& txn_manager, Thread * h_thd) {
...
void tpcc_wl::init_tab_item() {
	for (uint32_t i = 1; i <= g_max_items; i++) {
		...
		row->set_value((ColumnId)I_DATA, data);
		
		insert_into_index(i_item, i, row, 0);
	}
}
...
void tpcc_wl::init_tab_wh(uint32_t wid) {
	...
	Row * row;
	uint64_t row_id;
	t_warehouse->new_row(row, 0, row_id);
	...
	insert_into_index(i_warehouse, wid, row, wh_to_part(wid));
	return;
}
...
void tpcc_wl::init_tab_dist(uint64_t wid) {
	for (uint64_t did = 1; did <= DIST_PER_WARE; did++) {
		Row * row;
		uint64_t row_id;
		t_district->new_row(row, 0, row_id);
		...
		insert_into_index(i_district, distKey(did, wid), row, wh_to_part(wid));
	}
}
...
#endif
		insert_into_index(i_stock, stockKey(sid, wid), row, wh_to_part(wid));
	}
}
...
void tpcc_wl::init_tab_cust(uint64_t did, uint64_t wid) {
	...
	for (uint32_t cid = 1; cid <= g_cust_per_dist; cid++) {
		Row * row;
		uint64_t row_id;
		t_customer->new_row(row, 0, row_id);
		...
		insert_into_index(i_customer_last, key, row, wh_to_part(wid));
		key = custKey(cid, did, wid);
		insert_into_index(i_customer_id, key, row, wh_to_part(wid));
	}
}
...
void tpcc_wl::init_tab_hist(uint64_t c_id, uint64_t d_id, uint64_t w_id) {
	Row * row;
	uint64_t row_id;
	t_history->new_row(row, 0, row_id);
...
void tpcc_wl::init_tab_order(uint64_t did, uint64_t wid) {
	...
	for (uint32_t oid = 1; oid <= g_cust_per_dist; oid++) {
		...
		for (uint32_t ol = 1; ol <= o_ol_cnt; ol++) {
			t_orderline->new_row(row, 0, row_id);
		...
		// NEW ORDER
		if (oid > 2100) {
			t_neworder->new_row(row, 0, row_id);

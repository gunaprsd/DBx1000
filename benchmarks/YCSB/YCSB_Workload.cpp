...
int YCSB_Workload::next_tid;

Status YCSB_Workload::initialize() 
{
	Workload::initialize();
	next_tid = 0;
...
Status YCSB_Workload::initialize_schema(string schema_file) 
{
...
int YCSB_Workload::key_to_part(uint64_t key) 
{
	uint64_t rows_per_part = g_synth_table_size / g_part_cnt;
	return key / rows_per_part;
}

Status YCSB_Workload::initialize_table() 
{
	Status rc;
    ...
    while (true) {
    	for (UInt32 part_id = 0; part_id < g_part_cnt; part_id ++) {
			...
			assert(rc == OK);
			Key primary_key = total_row;
			new_row->set_primary_key(primary_key);
            new_row->set_value((ColumnId)0, primary_key);
...
void YCSB_Workload::init_table_parallel() 
{
...
void * YCSB_Workload::init_table_slice() 
{
...
Status YCSB_Workload::get_txn_manager(TransactionManager *& txn_manager, Thread * h_thd)
{
	txn_manager = (YCSB_TxnMan *)
		_mm_malloc( sizeof(YCSB_TxnMan), 64 );
	new(txn_manager) YCSB_TxnMan();

...
void execute_ycsb_benchmark() {
    struct YCSBBenchmarkConfig config = {.table_size = FLAGS_ycsb_table_size,
    ...
    if (FLAGS_task == "partition") {
        OfflinePartitioner<ycsb_params> scheduler(FLAGS_input_file, FLAGS_threads,
                                                  FLAGS_output_file);
        scheduler.schedule();
    } else if (FLAGS_task == "generate") {
        YCSBWorkloadGenerator generator(config, FLAGS_threads, FLAGS_size_per_thread,
                                        FLAGS_output_file);
        generator.generate();
    } else if (FLAGS_task == "execute") {
        YCSBExecutor executor(config, FLAGS_input_file, FLAGS_threads);
        executor.execute();
        executor.release();
    }
}
...
void execute_tpcc_benchmark() {
    struct TPCCBenchmarkConfig config = {.num_warehouses = FLAGS_tpcc_num_wh,
    ...
    if (FLAGS_task == "partition") {
        OfflinePartitioner<tpcc_params> scheduler(FLAGS_input_file, FLAGS_threads,
                                                  FLAGS_output_file);
        scheduler.schedule();
    } else if (FLAGS_task == "generate") {
        TPCCWorkloadGenerator generator(config, FLAGS_threads, FLAGS_size_per_thread,
                                        FLAGS_output_file);
        generator.generate();
    } else if (FLAGS_task == "execute") {
        TPCCExecutor executor(config, FLAGS_input_file, FLAGS_threads);
        executor.execute();
        executor.release();
    }
}

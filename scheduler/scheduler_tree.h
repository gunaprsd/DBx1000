...
template<typename T>
class SchedulerTree : public ITransactionQueue<T> {
...
protected:
	...
	bool is_queue_closed(Node *node) {
		return node->head == CLOSED;
	}

	bool is_parent_closed(Node* node) {
		return node->parent == CLOSED;
	}

	bool try_close_parent(Node* node) {
		return ATOM_CAS(node->parent, nullptr, CLOSED);
	}
	...
	void internal_add(Query<T> *txn, ReadWriteSet &rwset) {
		...
		if(root_nodes.size() == 1) {
			...
			for(uint32_t i = 0; i < rwset.num_accesses; i++) {
				auto key = rwset.accesses[i].key;
				data_nodes[key] = static_cast<uint64_t>(root_node);
			}

...
Status Row::initialize(Table * host_table, uint64_t part_id, uint64_t row_id)
{
	table = host_table;
	_row_id = row_id;
	_part_id = part_id;
...
Status Row::get_row(access_t type, txn_man * txn, Row *& row)
{
	Status rc = OK;
...
// (cf. row_ts.cpp)
void Row::return_row(access_t type, txn_man * txn, Row * row) {	
#if CC_ALG == WAIT_DIE || CC_ALG == NO_WAIT || CC_ALG == DL_DETECT

...
#define DECL_SET_VALUE(type) \
	void set_value(ColumnId col_id, type value);

#define SET_VALUE(type) \
	void set_value(ColumnId col_id, type value) { set_value(col_id, &value); }

#define DECL_GET_VALUE(type)\
	void get_value(ColumnId col_id, type & value);

#define GET_VALUE(type)\
	void Row::get_value(ColumnId col_id, type & value) {\
...
public:
	...
	uint64_t 		get_field_cnt() { return get_schema()->num_columns; }
	...
	void 			set_value(ColumnId id, void * ptr) { set_data((char*)ptr, GET_POS(id), GET_SIZE(id));  }
	void 			set_value(ColumnId id, void * ptr, int size) { set_data((char*)ptr, GET_POS(id), size); }
	void 			set_value(const char * col_name, void * ptr) { set_value( GET_ID(col_name), ptr); }
	char * 			get_value(ColumnId id) { return &data[GET_POS(id)]; }
	char * 			get_value(char * col_name) { return &data[ GET_POS( GET_ID(col_name) ) ]; }
	

	/* Typed Data Modifiers */
	void 			set_value(ColumnId col_id, uint64_t value) { set_value(col_id, & value); }
	void 			set_value(ColumnId col_id, int64_t value) { set_value(col_id, & value); }
	void 			set_value(ColumnId col_id, uint32_t value) { set_value(col_id, & value); }
	void 			set_value(ColumnId col_id, int32_t value) { set_value(col_id, & value); }
	void 			set_value(ColumnId col_id, double value) { set_value(col_id, & value); }

	void 			get_value(ColumnId col_id, uint64_t & value) { value = *(uint64_t*) & data[GET_POS(col_id)]; }
	void 			get_value(ColumnId col_id, int64_t & value) { value = *(int64_t*) & data[GET_POS(col_id)]; }
	void 			get_value(ColumnId col_id, uint32_t & value) { value = *(uint32_t*) & data[GET_POS(col_id)]; }
	void 			get_value(ColumnId col_id, int32_t & value) { value = *(int32_t*) & data[GET_POS(col_id)]; }
	void 			get_value(ColumnId col_id, double & value) { value = *(double*) & data[GET_POS(col_id)]; }

...
void Manager::init() {
	...
	*timestamp = 1;
	...
	_last_epoch_update_time = (Time *) _mm_malloc(sizeof(uint64_t), 64);
	_epoch = 0;
	_last_epoch_update_time = 0;
	all_ts = (Time volatile **) _mm_malloc(sizeof(Time *) * g_thread_cnt, 64);
	for (uint32_t i = 0; i < g_thread_cnt; i++) 
		all_ts[i] = (Time *) _mm_malloc(sizeof(Time), 64);

	_all_txns = new TransactionManager * [g_thread_cnt];
...
Time Manager::get_min_ts(uint64_t tid) {
...
void Manager::add_ts(uint64_t thd_id, Time ts) {
	assert( ts >= *all_ts[thd_id] || 
		*all_ts[thd_id] == UINT64_MAX);
	*all_ts[thd_id] = ts;
}

void Manager::set_txn_man(TransactionManager * txn) {
	int thd_id = txn->get_thread_id();

...
template <typename T> class QueryList : public BaseQueryList {
public:
  ...
  bool done() override { return (current == num_queries); }
...
template <typename T> class QueryMatrix : public BaseQueryMatrix {
public:
  ...
  void get(uint32_t i, uint32_t j, BaseQuery **query) override {
    *query = &queries[i][j];
  }

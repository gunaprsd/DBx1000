	...
	void connect() {
		...
		while(!query_iterator->done()) {
			...
			for(uint64_t i = 0; i < rwset.num_accesses; i++) {
				...
				BaseQuery** incoming_loc = reinterpret_cast<BaseQuery **>(db->data_next_pointer[rwset.accesses[i].key]);
				while(!added) {
					BaseQuery*** incoming_loc_loc = &incoming_loc;
					if(__sync_bool_compare_and_swap(incoming_loc_loc, incoming_loc, outgoing_loc)) {
						added = true;
						if(incoming_loc != 0) {
							* incoming_loc = query;
						}

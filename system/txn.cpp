...
Database *txn_man::get_db() { return database; }

uint64_t txn_man::get_thd_id() { return thread_id; }
...
void txn_man::cleanup(RC rc) {
  ...
  for (int rid = row_cnt - 1; rid >= 0; rid--) {
...
#if (CC_ALG == NO_WAIT || CC_ALG == NONE || CC_ALG == DL_DETECT) &&            \
    ...
    if (ROLL_BACK && type == XP &&
        (CC_ALG == DL_DETECT || CC_ALG == NO_WAIT || CC_ALG == NONE ||
         CC_ALG == WAIT_DIE)) {
...
#elif (CC_ALG == DL_DETECT || CC_ALG == NO_WAIT || CC_ALG == NONE ||           \
       CC_ALG == WAIT_DIE)
    access->orig_data = (row_t *)_mm_malloc(sizeof(row_t), 64);
    access->orig_data->init(MAX_TUPLE_SIZE);
#endif
...
#if ROLL_BACK && (CC_ALG == DL_DETECT || CC_ALG == NO_WAIT ||                  \
                  CC_ALG == NONE || CC_ALG == WAIT_DIE)
  if (type == WR) {

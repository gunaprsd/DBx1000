...
void Stats_thd::init(uint64_t thd_id) {
    clear();
    all_debug1 = (uint64_t *)_mm_malloc(sizeof(uint64_t) * FLAGS_size_per_thread, 64);
    all_debug2 = (uint64_t *)_mm_malloc(sizeof(uint64_t) * FLAGS_size_per_thread, 64);
}
...
void Stats::init() {
    if (!STATS_ENABLE)
        return;
    _stats = (Stats_thd **)_mm_malloc(sizeof(Stats_thd *) * FLAGS_threads, 64);
    tmp_stats = (Stats_tmp **)_mm_malloc(sizeof(Stats_tmp *) * FLAGS_threads, 64);
    dl_detect_time = 0;
    dl_wait_time = 0;
    deadlock = 0;
    cycle_detect = 0;
...
void Stats::print() {
    ...
    double total_union = 0;
    double total_find = 0;
    double total_blocked = 0;
    double total_execute = 0;
    for (uint64_t tid = 0; tid < FLAGS_threads; tid++) {
        ...
        total_union += _stats[tid]->time_union;
        total_find += _stats[tid]->time_find;
        total_blocked += _stats[tid]->time_blocked;
        total_execute += _stats[tid]->time_execute;

        printf("[tid=%ld] txn_cnt=%ld, abort_cnt=%ld, time_union=%lf, time_find=%lf, "
               "time_blocked=%lf, time_execute=%lf, run_time=%lf\n",
               tid, _stats[tid]->txn_cnt, _stats[tid]->abort_cnt, _stats[tid]->time_union / BILLION,
               _stats[tid]->time_find / BILLION, _stats[tid]->time_blocked / BILLION,
               _stats[tid]->time_execute / BILLION, _stats[tid]->run_time / BILLION);
    }
    ...
    if (output_file != NULL) {
        ...
        fprintf(outf,
                "[summary] txn_cnt=%lu, abort_cnt=%lu"
    ...
    printf("[summary] txn_cnt=%lu, abort_cnt=%lu"
           ", run_time=%f, throughput=%f, time_wait=%f, time_ts_alloc=%f"
           ", time_man=%f, time_index=%f, time_abort=%f, time_cleanup=%f, "
           "latency=%f"
           ", deadlock_cnt=%ld, cycle_detect=%ld, dl_detect_time=%f, "

...
template <typename T> class WorkloadLoader {
  public:
    WorkloadLoader(const string &file_path) : _file_path(file_path) {}

    virtual ~WorkloadLoader() {
        delete[] _queries;
    }
    ...
    void load() {
    	if(_queries == nullptr) {
		    ...
		    auto num_queries_read = fread(_queries, sizeof(Query<T>), static_cast<size_t>(num_queries_to_read), file);

...
void
ParallelWorkloadPartitioner::initialize(BaseQueryMatrix * queries,
                                        uint64_t max_size,
  ...
  _current_parts = reinterpret_cast<idx_t *>(
                     malloc(sizeof(idx_t) * _max_size));
...
void ParallelWorkloadPartitioner::partition_per_iteration() {
  ...
  if (_parallelism > 1) {
    ...
    start_time = get_server_clock();
    ParMETISGraphPartitioner::compute_partitions(
                                graph,
                                _num_arrays,
                                _current_parts);
    end_time = get_server_clock();
...
void * ParallelWorkloadPartitioner::parallel_create_graph_helper(void *data) {
  ...
  auto creator  = reinterpret_cast<METIS_CSRGraphCreator *>(
                    threadLocalData->fields[2]);
...
void *
ParallelWorkloadPartitioner::compute_statistics_helper(void * data) {
  auto threadLocalData = reinterpret_cast<ThreadLocalData *>(data);
  auto partitioner = reinterpret_cast<ParallelWorkloadPartitioner *>(
                       threadLocalData->fields[0]);
  auto thread_id = (uint32_t) threadLocalData->fields[1];
...
void ParallelWorkloadPartitioner::write_post_partition_file() {
  char file_name[100];
  snprintf(file_name, sizeof(file_name),
            "post_partition_%d.txt", _current_iteration);
  ...
  for (auto i = 0u; i < _num_arrays; i++) {
    ...
    fprintf(post_partition_file, "Core\t:%d\tNum Queries\t:%ld\n",
            static_cast<int32>(i),
            static_cast<int64>(num_queries));

    for (auto j = _tmp_array_sizes[i];
         j < (uint32_t)_tmp_queries[i].size(); j++) {
...
void ParallelWorkloadPartitioner::print_partition_summary() {
  ...
  printf("%-30s: %-10lu --> %lu\n", "Cross-Core Edges",
         ...
         _total_post_cross_core_edges);
  printf("%-30s: %-10lu --> %lu\n", "Cross-Core Weights",
         _total_pre_cross_core_weight,
         _total_post_cross_core_weight);
...
void ParallelWorkloadPartitioner::print_execution_summary() {
  ...
  printf("%-25s :: total: %10lf, avg: %10lf\n", "Obtain Data Statistics",
        ...
        data_statistics_duration / num_iterations);
  printf("%-25s :: total: %10lf, avg: %10lf\n", "Graph Structures Init",
        graph_init_duration,
        graph_init_duration / num_iterations);
  printf("%-25s :: total: %10lf, avg: %10lf\n", "Graph Clustering",
         partition_duration,
         partition_duration / num_iterations);
  printf("%-25s :: total: %10lf, avg: %10lf\n", "Shuffle Duration",
         shuffle_duration,
         shuffle_duration / num_iterations);
  printf("************************************************* \n");
}

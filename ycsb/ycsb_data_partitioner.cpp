...
void YCSBDataPartitioner::initialize(BaseQueryMatrix *queries, uint64_t max_cluster_graph_size, uint32_t parallelism,
																		 const char *dest_folder_path) {
	DataPartitioner::initialize(queries, max_cluster_graph_size, parallelism, dest_folder_path);
...
void YCSBDataPartitioner::partition_per_iteration() {
	...
	uint64_t start_time, end_time; double duration;
...
void YCSBDataPartitioner::per_thread_write_to_file(uint32_t thread_id, FILE *file) {
 ...
 * (1) assigns a batch-specific id to each data item that is being accessed in the batch
 * (2) computes statistics on data items
 */
...
void YCSBDataPartitioner::second_pass() {
	...
	for(auto txn_id = 0u; txn_id < _max_size; txn_id++) {
		...
		for(auto j = 0u; j  < params->request_cnt; j++) {
			...
			adjwgt.push_back(info->compute_edge_weight());
		}
	}
}
	...
	for(auto txn_id = 0u; txn_id < _max_size; txn_id++) {
		...
		for(auto j = 0u; j  < params->request_cnt; j++) {
			...
			if(info->id == next_data_id) {
				...
				adjwgt.insert(adjwgt.end(), info->txns.size(), info->compute_edge_weight());

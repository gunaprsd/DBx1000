...
void YCSBWorkloadGenerator::gen_single_partition_requests(uint64_t thread_id, ycsb_query *query) {
    ...
    uint32_t part_id = static_cast<uint32_t>(_random.nextInt64(thread_id) % config.num_partitions);
    uint64_t req_id = 0;
...
YCSBExecutor::YCSBExecutor(const YCSBBenchmarkConfig &config, const string &folder_path,
    ...
    _scheduler = new CCBFSScheduler<ycsb_params>(num_threads, &_db);
}

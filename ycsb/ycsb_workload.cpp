...
YCSBWorkloadGenerator::YCSBWorkloadGenerator(const YCSBBenchmarkConfig &_config,
    ...
    : ParallelWorkloadGenerator<ycsb_params>(num_threads, size_per_thread, folder_path),
      config(_config), zipfian(_config.table_size / _config.num_partitions, _config.num_partitions,
 ...
 */
void YCSBWorkloadGenerator::gen_single_partition_requests(uint64_t thread_id, ycsb_query *query) {
    set<uint64_t> all_keys;
    uint64_t max_row_id = config.table_size / config.num_partitions;
    uint32_t part_id = UINT32_MAX;
    if(FLAGS_generate_partitioned) {
        part_id = static_cast<uint32_t>(thread_id);
    } else {
...
void YCSBWorkloadGenerator::gen_multi_partition_requests(uint64_t thread_id, ycsb_query *query) {
    ...
    for (uint32_t i = 0; i < YCSB_NUM_REQUESTS; i++) {
        ...
        if (_random.nextDouble(thread_id) < config.remote_access_percent) {
            ...
            part_id =
                (thread_id * config.num_local_partitions) + (rint64 % config.num_local_partitions);
        }
...
YCSBExecutor::YCSBExecutor(const YCSBBenchmarkConfig &config, const string &input_file,
    ...
    if(FLAGS_scheduler_type == "online_batch") {
        ...
        _scheduler = new OnlineBatchSchedulerV4<ycsb_params>(num_threads, FLAGS_scheduler_batch_size, &_db);
    } else {
